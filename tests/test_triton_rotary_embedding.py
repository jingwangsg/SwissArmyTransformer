...
from sat.model.position_embedding.rotary_embeddings_original import (
    RotaryEmbedding, apply_rotary_pos_emb_bhs)
...
if __name__ == "__main__":
    ...
    llama_rotary = SatRotaryEmbedding(
        rotary_dim, base=10000, precision=torch.bfloat16, learnable=False, device=device
    )
    ...
    rotary_emb_chatglm = FastRotaryEmbedding(
        dim=headdim // 2, interleaved=True, device=device
    )
    ...
    st = time.time()
    for i in range(1000):
        q_triton, k_triton = rotary_emb_llama(
            query_l, key_l, position_ids, max_seqlen=seqlen
        )
    torch.cuda.synchronize()
    tt = time.time() - st
    ...
    st = time.time()
    for i in range(1000):
        q_triton, k_triton = rotary_emb_chatglm(
            query_c, key_c, position_ids, max_seqlen=seqlen
        )
    torch.cuda.synchronize()
    tt = time.time() - st
    print("now time is ", tt)
