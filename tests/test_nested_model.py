...
from sat.model import BaseMixin, BaseModel, BertModel


class NestedModel(BaseModel):
    def __init__(self, args, *argv, **kwargs):
        super().__init__(args, *argv, **kwargs)
        self.net = BertModel(BertModel.get_args(**args.bert_args))

    @classmethod
    def add_model_specific_args(cls, parser):
        group = parser.add_argument_group(
            "Nested Example", "Nested Example Configurations"
        )
        group.add_argument(
            "--bert_args", type=json.loads, default={}, help="Bert Model Arguments"
        )
        return parser

    @BaseMixin.non_conflict
    def word_embedding_forward_default(
        self, input_ids, output_cross_layer, old_impl, **kw_args
    ):
        return old_impl(input_ids, **kw_args) + self.net(
...
def test_save_and_load():
    args = NestedModel.get_args(
        num_layers=2, bert_args={"hidden_size": 128, "num_layers": 3, "num_types": 2}
    )
    args.mode = "inference"
    ...
    assert os.path.exists(os.path.join(args.save, str(1), "mp_rank_00_model_states.pt"))
