...
def get_masks_and_position_ids(data, loss_mask=None, attention_mask=None, args=None):
    ...
    if attention_mask is None:
        attention_mask = torch.ones(
            (batch_size, seq_length, seq_length), device=data.device
        )
...
def get_batch(data_iterator, args, timers):
    ...
    attention_mask, loss_mask, position_ids = get_masks_and_position_ids(
        tokens, loss_mask=loss_mask, attention_mask=attention_mask, args=args
    )
    # Convert
    if args.fp16:
...
def create_dataset_function(path, args):
    ...
    def process_fn(row):
        return {"text": row[0], "loss_mask": np.array([1] * len(row[0]))}

    return FakeDataset(process_fn, x)


if __name__ == "__main__":
    from tests.test_nested_model import NestedModel

    sys.argv = sys.argv + [
        "--bert_args",
        '{"hidden_size": 128, "num_layers": 3, "num_types": 2}',
    ]
    ...
    NestedModel.add_model_specific_args(py_parser)
    known, args_list = py_parser.parse_known_args()
    args = get_args(
        args_list
        + "--train-data fake --num-workers 0 --save ./checkpoints/test_train_nested --train-iters 500".split()
    )
    args = argparse.Namespace(**vars(args), **vars(known))

    training_main(
        args,
        model_cls=NestedModel,
