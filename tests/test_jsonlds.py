...
from sat.data_utils import JsonlIterableDataset, make_loaders
...
args = get_args(
    [
        ...
        "/mnt/shared/img_datasets/text_dm/wudao/0.jsonl",
        "--train-data-weights",
        "2",
        "1",
        "--iterable-dataset",
...
def process_fn(src, seq_len):
    tokenizer = AutoTokenizer.from_pretrained(
        "/mnt/shared/official_pretrains/hf_home/chatglm2-6b",
    ...
    for x in src:
        ...
        if buffered_token_ids is None:
            ...
            buffered_token_ids = torch.cat(
                (buffered_token_ids, eos_id, tokenized_ids), dim=0
            )
        # yield per seq_len
...
def create_func(path, args):
    return JsonlIterableDataset(
        path, partial(process_fn, seq_len=512), seed=1, shuffle_buffer=1
    )


train_loader, val_loader, test_loader = make_loaders(
    args, create_dataset_function=create_func
)
