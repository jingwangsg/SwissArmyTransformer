...
def get_tokenizer(args=None, *, tokenizer_type=None, outer_tokenizer=None):
    ...
    if outer_tokenizer is not None:  # set 1
        ...
        print_rank0(
            "> Set tokenizer as an outer_tokenizer! Now you can get_tokenizer() everywhere."
        )
    ...
    if (
        hasattr(get_tokenizer, "tokenizer_type")
        and tokenizer_type == get_tokenizer.tokenizer_type
    ):  # the same as last
        return get_tokenizer.tokenizer
    ...
    if tokenizer_type.startswith("cogview"):  # or cogview_ICE
        ...
        if tokenizer_type == "glm_GPT2BPETokenizer":
            ...
            get_tokenizer.tokenizer = GPT2BPETokenizer(
                args.tokenizer_model_type, **kwargs
            )
        elif tokenizer_type == "glm_ChineseSPTokenizer":
            from .glm import ChineseSPTokenizer

            get_tokenizer.tokenizer = ChineseSPTokenizer(
                args.tokenizer_model_type, **kwargs
            )
        elif tokenizer_type == "glm_BertWordPieceTokenizer":
            from .glm import BertWordPieceTokenizer

            get_tokenizer.tokenizer = BertWordPieceTokenizer(
                args.tokenizer_model_type, **kwargs
            )
    ...
    #         get_tokenizer.tokenizer = HFT5Tokenizer(args.tokenizer_model_type, cache_dir=args.cache_dir)
        ...
        try:
            get_tokenizer.tokenizer = AutoTokenizer.from_pretrained(
                tokenizer_type, trust_remote_code=True
            )
        except OSError as e:
            print_rank0(
                f"Cannot find {tokenizer_type} from Huggingface or sat. Creating a fake tokenizer..."
            )
            assert args.vocab_size > 0
            get_tokenizer.tokenizer = FakeTokenizer(args.vocab_size)
            return get_tokenizer.tokenizer
    print_rank0(
        f"> Set tokenizer as a {tokenizer_type} tokenizer! Now you can get_tokenizer() everywhere."
    )
    return get_tokenizer.tokenizer
