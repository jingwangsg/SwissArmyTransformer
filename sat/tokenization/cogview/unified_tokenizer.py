...
class UnifiedTokenizer(object):
    def __init__(self, img_tokenizer_path, txt_tokenizer_type, device):
        ...
        self.img_tokenizer = VQVAETokenizer(
            model_path=img_tokenizer_path, device=self.device
        )
        ...
        self.command_tokens = {
            k: v + self.num_tokens for k, v in self.raw_command_tokens
        }
        ...
        try:
            for x in ids:
                if self.num_tokens - len(self.raw_command_tokens) <= x:
                    # command tokens
                    token = self.raw_command_tokens[
                        x - (self.num_tokens - len(self.raw_command_tokens))
                    ][0]
    ...
    def wrap_code(self, code, idx=1):
        ...
        if isinstance(code, list):
            return (
                [self.command_tokens[prefix], self.command_tokens[boi]]
                + code
                + [self.command_tokens[eoi]]
            )
