...
def new_module(config):
    ...
    model = getattr(importlib.import_module(module, package=__package__), cls)(
    ...
    if "ckpt" in config:
        ...
        if prefix is not None:
            ckpt = {
                k[len(prefix) + 1 :]: v for k, v in ckpt.items() if k.startswith(prefix)
            }
...
def load_model_default(device=0, path="pretrained/vqvae/l1+ms-ssim+revd_percep.pt"):
...
def test_decode(configs, testcase, device=0, output_path=None):
    ...
    if output_path is None:
        output_path = os.path.join(
            "sample", f"{datetime.now().strftime('%m-%d-%H-%M-%S')}.jpg"
        )
    ...
    if type(testcase) is str:
        testcase = torch.load(testcase, map_location=torch.device(device))[
            :, -1024:
        ].contiguous()
...
def code2img(model, code):
    ...
    with torch.no_grad():
        ...
        out = out * torch.tensor([0.30379, 0.32279, 0.32800], device=out.device).view(
            1, -1, 1, 1
        ) + torch.tensor([0.79093, 0.76271, 0.75340], device=out.device).view(
            1, -1, 1, 1
        )
    return out
