...
from .utils import VocabUtility, divide, unscaled_init_method
    ...
    def partition(self, new_model_parallel_size=None, full_weight=None):
        ...
        for rank in range(new_model_parallel_size):
            vocab_start_index, vocab_end_index = (
                VocabUtility.vocab_range_from_global_vocab_size(
                    self.num_embeddings, rank, new_model_parallel_size
                )
            )
            weight = torch.clone(
...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def __init__(
        ...
        if bias:
            self.bias = Parameter(
                torch.empty(
                    self.output_size_per_partition, dtype=params_dtype, device=device
                )
            )
    ...
    def repartition(self):
        ...
        self.output_size_per_partition = divide(
            self.output_size, get_model_parallel_world_size()
        )
        ...
        strides = (
            [1] * self.stride if isinstance(self.stride, int) else self.stride
        )  # int means equal number of qkv, or ratios
        ...
        for i in range(len(strides)):
            strided_weights.append(
                self.weight[_acm : _acm + factor * strides[i], :].detach()
            )
        ...
        new_weight = (
            torch.cat(
                [
                    strided_weight[
                        (strided_weight.shape[0] // mp_size)
                        * mp_rank : (strided_weight.shape[0] // mp_size)
                        * (mp_rank + 1)
                    ]
            ...
            .contiguous()
            .view(self.output_size_per_partition, self.input_size)
        )
        ...
        if self.bias is not None and self.bias.numel() != 0:
            ...
            for i in range(len(strides)):
                strided_biases.append(
                    self.bias[_acm : _acm + factor * strides[i]].detach()
                )
                _acm += factor * strides[i]
            new_bias = (
                torch.cat(
                    [
                        strided_bias[
                            (strided_bias.shape[0] // mp_size)
                            * mp_rank : (strided_bias.shape[0] // mp_size)
                            * (mp_rank + 1)
                        ]
                ...
                .contiguous()
                .view(self.output_size_per_partition)
            )
    ...
    def partition(self, new_model_parallel_size=None, full_weight=None):
        ...
        for i in range(len(strides)):
            strided_weights.append(
                full_weight[_acm : _acm + factor * strides[i], :].detach()
            )
            _acm += factor * strides[i]
        ...
        for rank in range(new_model_parallel_size):
            ...
            new_weight = (
                ...
                .contiguous()
                .view(output_size_per_partition, self.input_size)
            )
            ...
            if flag == 2 and self.bias is not None and self.bias.numel() != 0:
                new_bias = (
                    ...
                    .contiguous()
                    .view(output_size_per_partition)
                )
...
class RowParallelLinear(torch.nn.Module):
    ...
    def __init__(
        ...
        if bias:
            self.bias = Parameter(
                torch.empty(self.output_size, dtype=params_dtype, device=device)
            )
            # Always initialize bias to zero.
            with torch.no_grad():
                self.bias.zero_()
    ...
    def forward(self, input_):
        ...
        if self.final_bias or self.bias is None:
            ...
            output_parallel = F.linear(
                input_parallel, self.weight, self.bias / get_model_parallel_world_size()
            )
    ...
    def repartition(self):
        ...
        self.input_size_per_partition = divide(
            self.input_size, get_model_parallel_world_size()
        )
        mp_rank = get_model_parallel_rank()
        self.original_weight = self.weight
        self.weight = torch.nn.Parameter(
            torch.clone(
                self.weight[
                    :,
                    mp_rank
                    * self.input_size_per_partition : (mp_rank + 1)
                    * self.input_size_per_partition,
                ],
            ).detach()
        )
    ...
    def partition(self, new_model_parallel_size=None, full_weight=None):
        assert (
            self.input_size_per_partition == self.input_size or full_weight is not None
        )
        flag = 1
        if full_weight is None:
            full_weight = self.weight
        ...
        for rank in range(new_model_parallel_size):
            ...
            weight = torch.clone(
                full_weight[
                    :,
                    mp_rank
                    * input_size_per_partition : (mp_rank + 1)
