...
from sat.mpu import (ColumnParallelLinear, RowParallelLinear,
                     VocabParallelEmbedding, destroy_model_parallel,
...
def mp_split_model(model, new_model_parallel_size):
    ...
    """
    from sat.model.transformer import CrossAttention, SelfAttention
...
from .initialize import (get_model_parallel_group, get_model_parallel_src_rank,
                         get_model_parallel_world_size, get_node_group,
                         get_node_src_rank, get_node_world_size)
...
def mp_split_model_rank0(model, model_full, use_node_group=True):
    ...
    local_world_size = (
        get_node_world_size() if use_node_group else get_model_parallel_world_size()
    )

    def iter_repartition(new_model, module):
        for (new_name, sub_new_model), (name, sub_module) in zip(
            new_model.named_children(), module.named_children()
        ):
            if isinstance(
                sub_module,
                ...
                for i in range(local_world_size):
                    if i == 0:
                        sub_new_model.weight.data.copy_(
                            new_weights[src % len(new_weights)]
                        )
                    else:
                        torch.distributed.send(
                            new_weights[(src + i) % len(new_weights)].cuda(), src + i
                        )
                if new_biases:
                    for i in range(local_world_size):
                        if i == 0:
                            sub_new_model.bias.data.copy_(
                                new_biases[src % len(new_weights)]
                            )
                        else:
                            torch.distributed.send(
                                new_biases[(src + i) % len(new_biases)].cuda(), src + i
                            )
            else:
                for (nn, np), (n, p) in zip(
                    sub_new_model.named_parameters(recurse=False),
                    sub_module.named_parameters(recurse=False),
                ):
...
def mp_merge_model_rank0(model, model_full):
    assert (
        get_model_parallel_world_size() == torch.distributed.get_world_size()
    ), "Merging model is only supported for model_parallel_size == world_size!"
        ...
        for (new_name, sub_new_model), (name, sub_module) in zip(
            ...
            if isinstance(
                ...
                torch.distributed.gather(
                    sub_new_model.weight.data, gather_list=new_weights, dst=0
                )
                if new_biases:
                    new_biases = [x.to(sub_new_model.weight.device) for x in new_biases]
                    torch.distributed.gather(
                        sub_new_model.bias.data, gather_list=new_biases, dst=0
                    )
                sub_module.merge(
                    [torch.clone(x.cpu()).detach() for x in new_weights],
                    [torch.clone(x.cpu()).detach() for x in new_biases],
                )
                del new_weights
                if new_biases:
                    del new_biases
