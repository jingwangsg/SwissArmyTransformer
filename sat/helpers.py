...
def print_parser(parser, help_width=32):
    ...
    print(
        f"{'Argument'.ljust(max_name_len)}  Help"
        + " " * (help_width - 4)
        + f"{'Type'.ljust(8)}    Default"
    )
    print("-" * (max_name_len + 56))
    ...
    for arg_name, arg_help, arg_type, arg_default in argument_list:
        ...
        for i, line in enumerate(wrapped_help):
            if i == 0:
                print(
                    f"{name_str}  {line.ljust(help_width)}  {type_str}  {arg_default}"
                )
            else:
...
def list_avail_pretrained():
    ...
    print(
        'Available pretrained models (example: sat.AutoModel.from_pretrained("roberta-base")):'
    )
    print_aligned_string_list(model_list)
    return model_list
...
def debug_param(name, param):
    print_all(
        f"param: {name}, min: {param.min()}, max: {param.max()}, mean: {param.mean()}, std: {param.std()}, scale: {param.abs().mean()}, first5: {param.flatten()[:5]}, last5: {param.flatten()[-5:]}"
    )
    ...
    if g is not None:
        print_all(
            f"grad: {name}, min: {g.min()}, max: {g.max()}, mean: {g.mean()}, std: {g.std()}, scale: {g.abs().mean()}, first5: {g.flatten()[:5]}, last5: {param.flatten()[-5:]}"
        )
    s = safe_get_full_optimizer_state(param, "exp_avg")
    if s is not None:
        print_all(
            f"state: {name}, min: {s.min()}, max: {s.max()}, mean: {s.mean()}, std: {s.std()}, scale: {s.abs().mean()}, first5: {s.flatten()[:5]}"
        )
...
def check_if_zero3(args):
    return (
        ...
        and (args.deepspeed_config.get("zero_optimization", {}).get("stage", 0) >= 3)
    )
