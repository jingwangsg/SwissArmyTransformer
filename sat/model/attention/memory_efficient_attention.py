...
class TransposedMemoryEfficientAttentionMixin(BaseMixin):
    ...
    def attention_forward(self, hidden_states, mask, **kw_args):
        ...
        if "attention_fn" in self.hooks and self.hooks["attention_fn"] != attention_fn:
            ...
            print_rank0(
                "Dangerous! Memory efficient attention uses B L H D format, different from the default attention! You should not change attention_fn except you already noticed it.",
                level="warning",
            )
        ...
        query_layer = mixed_query_layer.view(
            mixed_query_layer.shape[0],
            mixed_query_layer.shape[1],
            self.num_attention_heads_per_partition,
            self.hidden_size_per_attention_head,
        )
        key_layer = mixed_key_layer.view(
            mixed_key_layer.shape[0],
            mixed_key_layer.shape[1],
            self.num_attention_heads_per_partition,
            self.hidden_size_per_attention_head,
        )
        value_layer = mixed_value_layer.view(
            mixed_value_layer.shape[0],
            mixed_value_layer.shape[1],
            self.num_attention_heads_per_partition,
            self.hidden_size_per_attention_head,
        )
