...
from sat.model.transformer import ColumnParallelLinear, RowParallelLinear
        ...
        def _load_from_state_dict(
            ...
            if prefix + "weight" in state_dict:
                ...
                if self.weight.data.dtype == torch.uint8:
                    quant_dict = {
                        k[len(prefix + "weight.") :]: v
                        for k, v in state_dict.items()
                        if k.startswith(prefix + "weight.")
                    }
                    self.weight.quant_state = QuantState.from_dict(
                        quant_dict, device=self.weight.data.device
                    )
            if prefix + "bias" in state_dict:
                self.bias.data.copy_(state_dict[prefix + "bias"])
...
class LoraLinear(nn.Module):
    def __init__(
        ...
        if type(partition) is int:
            self.matrix_A = HackParameterList(
                [
                    nn.Parameter(
                        torch.empty((r, original_obj.weight.shape[1]), dtype=dtype)
                    )
            ...
            self.matrix_B = HackParameterList(
                [
                    nn.Parameter(
                        torch.empty(
                            (original_obj.weight.shape[0] // partition, r), dtype=dtype
                        )
                    )
                    for _ in range(partition)
                ]
            ...
            new_sizes = [
                original_obj.weight.shape[0] // sum(partition) * i for i in partition
            ]
            self.matrix_A = HackParameterList(
                [
                    nn.Parameter(
            ...
            self.matrix_B = HackParameterList(
                [nn.Parameter(torch.empty((sz, r), dtype=dtype)) for sz in new_sizes]
            )
    ...
    def _load_from_state_dict(
        ...
        if prefix + "weight" in state_dict:
            ...
            self.original._load_from_state_dict(
                state_dict,
                prefix,
                local_metadata,
                strict,
    ...
    def forward(self, x):
        ...
        for mA, mB in zip(self.matrix_A, self.matrix_B):
            lora_outputs.append(
                (copy_to_model_parallel_region(x @ mA.T) @ mB.T) * self.scaling
            )
...
def replace_linear_with_lora(lin, partition, r, *args, **kw_args):
    ...
    new_layer = LoraLinear(
        original_cls, partition, in_dim, out_dim, r, *args, **kw_args, original_obj=lin
    )
...
def merge_linear_lora(lin):
    if lin.original.weight.data.dtype is not torch.uint8:
        ...
        new_lin = nn.Linear(
            in_dim, out_dim, dtype=weight.data.dtype, bias=lin.original.bias is not None
        )
    else:
        import bitsandbytes.functional as F

        weight = F.dequantize_fp4(
            lin.original.weight.data, lin.original.weight.quant_state
        ).to(lin.original.bias.data.dtype)
        out_dim, in_dim = weight.shape
    ...
    guess_type = (
        ...
        if lin.original.bias is not None
        else lin.original.weight.data.dtype
    )
...
class LoraMixin(BaseMixin):
    ...
    def reinit(self, parent_model):
        for i in self.layer_range:
            ...
            parent_model.transformer.layers[i].attention.query_key_value = (
                replace_linear_with_lora(
                    parent_model.transformer.layers[i].attention.query_key_value,
                    ...
                    out_size=(
                        None
                        if not parent_model.transformer.num_multi_query_heads
                        else parent_model.transformer.layers[
                            i
                        ].attention.inner_hidden_size
                        + parent_model.transformer.layers[
                            i
                        ].attention.hidden_size_per_attention_head
                        * parent_model.transformer.layers[
                            i
                        ].attention.num_multi_query_heads
                        * 2
                    ),
                )
            )
            if self.cross_attention and parent_model.transformer.layers[i].is_decoder:
                ...
                kv_size = (
                    parent_model.transformer.layers[i].cross_attention.inner_hidden_size
                    * 2
                    if not parent_model.transformer.cross_num_multi_query_heads
                    else parent_model.transformer.layers[
                        i
                    ].cross_attention.hidden_size_per_attention_head
                    * parent_model.transformer.layers[
                        i
                    ].cross_attention.cross_num_multi_query_heads
                    * 2
                )
                parent_model.transformer.layers[i].cross_attention.dense = (
                    replace_linear_with_lora(
                        parent_model.transformer.layers[i].cross_attention.dense,
                        1,
                        self.r,
                        ...
                        self.lora_dropout,
                        qlora=self.qlora,
                        in_size=parent_model.transformer.layers[
                            i
                        ].cross_attention.inner_hidden_size,
                        out_size=parent_model.transformer.hidden_size,
                    )
                )
                parent_model.transformer.layers[i].cross_attention.query = (
                    replace_linear_with_lora(
                        parent_model.transformer.layers[i].cross_attention.query,
                        1,
                        self.r,
                        ...
                        qlora=self.qlora,
                        in_size=parent_model.transformer.hidden_size,
                        out_size=parent_model.transformer.layers[
                            i
                        ].cross_attention.inner_hidden_size,
                    )
                )
                parent_model.transformer.layers[i].cross_attention.key_value = (
                    replace_linear_with_lora(
                        parent_model.transformer.layers[i].cross_attention.key_value,
                        2,
                        self.r,
                ...
                for name, child in model.named_children():
                    if name not in names:
                        if child in cache:
                            ...
                            new_child = replace_linear_with_nf4(
                                child, name=name, cache=cache
                            )
