...
class LoRAMixin(BaseMixin):
    def __init__(
        ...
        self.lora_linear = nn.ModuleList(
            [nn.ParameterDict() for layer_id in range(layer_num)]
        )
        matrices = ["Q", "K", "V", "O"]

        for i in layer_range:
            for matrix in matrices:
                self.lora_linear[i][matrix + "_A"] = nn.Parameter(
                    torch.zeros((r, hidden_size))
                )
                self.lora_linear[i][matrix + "_B"] = nn.Parameter(
                    torch.zeros((hidden_size, r))
                )
                nn.init.kaiming_uniform_(
                    self.lora_linear[i][matrix + "_A"], a=math.sqrt(5)
                )
    ...
    def attention_forward(self, hidden_states, mask, layer_id, **kw_args):
        ...
        if layer_id in self.layer_range:
            output = (
                output
                + (
                    self.lora_dropout(context_layer)
                    @ lora_layer["O_A"].T
                    @ lora_layer["O_B"].T
                )
                * self.scaling
