...
class PosMixin(InterpolatedPositionEmbeddingMixin):
    def __init__(self, hidden_size, old_property, property, init_method_std=0.02):
        super().__init__(
            hidden_size, old_property, property, init_method_std=init_method_std
        )
    ...
    def reinit(self, parent_model=None):
        ...
        self.transformer.position_embeddings = (
            torch.nn.Embedding(self.property.seq_len, old_weight.shape[1])
            .type(old_weight.dtype)
            .to(old_weight.device)
            .requires_grad_(False)
        )
        self.transformer.position_embeddings.weight.data = torch.Tensor(
            get_2d_sincos_pos_embed(
                self.hidden_size,
                self.property.grid_size,
                self.property.pre_len,
                self.property.post_len,
            )
        )
    ...
    def layer_forward(self, hidden_states, mask, *args, **kw_args):
        ...
        if kw_args["layer_id"] == 0:
            hidden_states, dic_buffer = self.after_position_forward(
                hidden_states, **kw_args
            )
            for k in dic_buffer:
...
class MAEEncoder(ViTModel):
    def __init__(self, args, transformer=None, layernorm_epsilon=1e-6):
        super().__init__(
            args, transformer=transformer, layernorm_epsilon=layernorm_epsilon
        )
        self.del_mixin("cls")
        self.del_mixin("pos_embedding")
        self.add_mixin(
            "pos_embedding",
            PosMixin(args.hidden_size, self.old_property, self.property),
        )
...
class MaskMixin(BaseMixin):
    def __init__(self, args):
        ...
        self.decoder_embed = nn.Linear(
            args.enc_hidden_size, args.hidden_size, bias=True
        )
        self.decoder_pred = nn.Linear(
            args.hidden_size, args.patch_size**2 * args.in_channels, bias=True
        )  # decoder to patch
    ...
    def word_embedding_forward(self, input_ids, **kwargs):
        ...
        mask_tokens = self.transformer.word_embeddings(input_ids).repeat(
            1, ids_restore.shape[1] + 1 - x.shape[1], 1
        )
...
class MAE(EncoderDecoderModel):
    def __init__(self, args, transformer=None, layernorm_epsilon=1e-6):
        ...
        decoder = MAEDecoder(
            dec_args, transformer=transformer, layernorm_epsilon=layernorm_epsilon
        )
        super().__init__(
            args, encoder=encoder, decoder=decoder, tie_word_embeddings=False
        )
    ...
    def forward(
        ...
        **kw_args
        ...
        encoder_outputs, *encoder_mems = self.encode(
            input_ids, enc_position_ids, enc_attention_mask, **kw_args
        )
        decoder_outputs, *decoder_mems = self.decode(
            input_ids,
            dec_position_ids,
            dec_attention_mask,
            encoder_outputs=encoder_outputs,
            ids_restore=encoder_mems[0]["ids_restore"],
            **kw_args
        )
