    ...
    def word_embedding_forward(self, input_ids, **kwargs):
        ...
        * (batch_size, pre_len+num_patches+post_len, hidden_size)
        ...
        pre_word_embeddings = self.transformer.word_embeddings(
            input_ids[:, : self.property.pre_len]
        )
        post_word_embeddings = self.transformer.word_embeddings(
            input_ids[
                :,
                self.property.pre_len : self.property.pre_len + self.property.post_len,
            ]
        )
...
class SwiGLUMixin(BaseMixin):
    def __init__(
        ...
        self.w2 = nn.ModuleList(
            [nn.Linear(in_features, hidden_features) for i in range(num_layers)]
        )

        self.act = act_layer()
        self.ffn_ln = nn.ModuleList(
            [LayerNorm(hidden_features, eps=eps) for i in range(num_layers)]
        )
...
from sat.model.position_embedding.vision_rotary_embeddings import \
...
class EVA2Model(BaseModel):
    def __init__(self, args, transformer=None, **kwargs):
        ...
        self.add_mixin(
            "patch_embedding",
            MaskedPatchEmbedMixin(args.in_channels, args.hidden_size, self.property),
        )
        ...
        self.add_mixin(
            "eva2-final", EVA2FinalMixin(args.predict_feature_dim, args.hidden_size)
        )
        self.add_mixin(
            "eva2-mlp",
            SwiGLUMixin(
        ...
        self.add_mixin(
            "eva2-attn",
            EVA2AttnMixin(args.hidden_size, args.num_attention_heads, self.property),
        )
