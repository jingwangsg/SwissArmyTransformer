...
from sat.model.transformer import split_tensor_along_last_dim, unscaled_init_method
...
class PositionEmbeddingMixin(BaseMixin):
    ...
    def reinit(self, parent_model=None):
        old_weights = self.transformer.position_embeddings.weight.data[
            self.reinit_slice
        ]
        old_len, hidden_size = old_weights.shape
        assert hidden_size == self.position_embeddings.weight.shape[-1]
        self.position_embeddings.weight.data.view(-1, old_len, hidden_size).copy_(
            old_weights
        )
...
class AttentionMixin(BaseMixin):
    ...
    def reinit(self, parent_model=None):
        ...
        for layer_id in range(self.num_layers):
            ...
            self.query_key_value[layer_id].weight.data.copy_(
                old_attention.query_key_value.weight.data
            )
            self.query_key_value[layer_id].bias.data.copy_(
                old_attention.query_key_value.bias.data
            )
...
class Cuda2dModel(BaseModel):
    def __init__(self, args, transformer=None):
        ...
        self.add_mixin(
            "extra_position_embedding",
            PositionEmbeddingMixin(additional_seqlen, args.hidden_size),
        )
        self.add_mixin(
            "attention_plus",
            AttentionMixin(num_layers=args.num_layers, hidden_size=args.hidden_size),
        )
    ...
    def position_embedding_forward(self, position_ids, **kw_args):
        ...
        position_embeddings = torch.cat(
            (
                ...
                self.get_mixin("extra_position_embedding").position_embeddings(
                    position_plus
                ),
            ),
            dim=-2,
    ...
    def attention_forward(
        self, hidden_states, mask, layer_id=None, log_attention_weights=None, **kw_args
    ):
        attn_module = self.transformer.layers[layer_id].attention
        # attention_plus on all layers
        query_key_value_plus = self.get_mixin("attention_plus").query_key_value[
            layer_id
        ]
...
def sparse_attention_2d_light(
    ...
    **kwargs
    ...
    q1 = (
        (
            q1.view(b, s1, n_head, h1 // n_head).permute(0, 2, 3, 1)
            / math.sqrt(h1 // n_head)
        )
        .contiguous()
    ...
    k1 = (
        k1.view(b, s1, n_head, h1 // n_head)
        .permute(0, 2, 3, 1)
        .contiguous()
        .view(b * n_head, h1 // n_head, l1, l1)
    )
    v1 = (
        v1.view(b, s1, n_head, h1 // n_head)
        .permute(0, 2, 3, 1)
        .contiguous()
        .view(b * n_head, h1 // n_head, l1, l1)
    )
    ...
    scores_1_to_0 = f_similar(
        q1, k0T, kernel_size2, kernel_size2, False
    )  # [b*n_head, l1, l1, field]
    scores_1 = torch.cat(
        (
    ...
    probs_1_to_1 = attention_probs1[:, :, -scores_1_to_1.shape[3] :].view_as(
        scores_1_to_1
    )
    context1_to_1 = f_weighting(
        v1, probs_1_to_1.contiguous(), kernel_size * 2 - 1, kernel_size, True
    )
    context1 = context1_to_1.view(b, n_head * h, l1**2)
    # weighting for cross attention
    probs_1_to_0 = attention_probs1[:, :, : scores_1_to_0.shape[3]].view_as(
        scores_1_to_0
    )
    v0_part = (
        v0[:, :, -(l0**2) :].transpose(-1, -2).contiguous().view(b * n_head, h, l0, l0)
    )
    context1_to_0 = f_weighting(
        v0_part, probs_1_to_0.contiguous(), kernel_size2, kernel_size2, False
    )
