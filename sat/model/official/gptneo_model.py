    ...
    def word_embedding_forward(self, input_ids, **kwargs):
        if "token_type_ids" in kwargs:
            return self.transformer.word_embeddings(
                input_ids
            ) + self.transformer.word_embeddings(kwargs["token_type_ids"])
        else:
...
class GPTNeoAttentionMixin(BaseMixin):
    def __init__(self, attention_types, window_size, max_positions):
        ...
        self.bias_global = torch.tril(
            torch.ones((max_positions, max_positions), dtype=torch.uint8)
        ).view(1, 1, max_positions, max_positions)
        self.bias_local = torch.bitwise_xor(
            self.bias_global, torch.tril(self.bias_global, -window_size)
        )
    ...
    def attention_fn(
        ...
        **kwargs,
        ...
        causal_mask = (
            bias[:, :, key_length - query_length : key_length, :key_length]
            .to(torch.bool)
        ...
        mask_value = torch.tensor(mask_value, dtype=attention_scores.dtype).to(
        ...
        if not (attention_mask.shape[-2] == 1 and (attention_mask > 0).all()):
            ...
            attention_scores = torch.where(
                attention_mask.to(attention_scores.device), attention_scores, mask_value
            )
    ...
    def __init__(self, args, transformer=None, **kwargs):
        ...
        self.add_mixin(
            ...
            GPTNeoAttentionMixin(
                args.attention_types, args.window_size, args.max_sequence_length
            ),
        )
