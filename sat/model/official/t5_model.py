...
from sat.model.transformer import MLP, CrossAttention, SelfAttention, standard_attention
...
from sat.mpu.utils import divide, unscaled_init_method
...
class T5AttentionMixin(BaseMixin):
    def __init__(
        self, relative_attention_num_buckets, num_attention_heads, is_decoder=False
    ):
        super().__init__()
    ...
    def final_forward(self, logits, **kwargs):
        ...
        if self.tie_word_embeddings:
            ...
            logits_parallel = F.linear(
                logits_parallel, self.transformer.word_embeddings.weight
            )
        else:
...
class T5GatedGeluMLPMixin(BaseMixin):
    ...
    def _init_weights(self, weight, **kwargs):
        torch.nn.init.normal_(
            weight, mean=0, std=self.init_method_std * (self.hidden_size**-0.5)
        )
...
class T5Model(EncoderDecoderModel):
    def __init__(self, args, **kwargs):
        ...
        self.encoder.add_mixin(
            "t5-attention",
            T5AttentionMixin(
                args.relative_attention_num_buckets, args.num_attention_heads
            ),
        )
        ...
        num_attention_heads = (
            ...
            if args.dec_num_attention_heads is not None
        ...
        self.decoder.add_mixin(
            ...
            T5DecoderFinalMixin(
                args.vocab_size,
                args.hidden_size,
                tie_word_embeddings=not args.no_share_embeddings,
            ),
        )
    ...
    def _init_weights(self, weight, module, name):
        ...
        if isinstance(module, MLP):
            if name == "dense_h_to_4h":
                torch.nn.init.normal_(
                    weight, mean=0, std=init_method_std * (module.hidden_size**-0.5)
                )
            elif name == "dense_4h_to_h":
                torch.nn.init.normal_(
                    weight,
                    mean=0,
                    std=init_method_std * (module.inner_hidden_size**-0.5),
                )
            else:
            ...
            if name == "query_key_value":
                ...
                torch.nn.init.normal_(
                    weight[: module.inner_hidden_size],
                    mean=0,
                    std=init_method_std
                    * (
                        (module.hidden_size * module.hidden_size_per_attention_head)
                        ** -0.5
                    ),
                )
            elif name == "dense":
