...
from sat import mpu, update_args_with_file
from sat.model.base_model import (BaseMixin, BaseModel, get_model,
                                  load_checkpoint, non_conflict)
from sat.model.mixins import BaseMixin
from sat.model.official.vit_model import ImagePatchEmbeddingMixin, ViTModel
...
class ImageEncoder(ViTModel):
    def __init__(
        self, args, layernorm_epsilon=1e-5, activation_func=QuickGELUActivation()
    ):
        super().__init__(
            args, layernorm_epsilon=layernorm_epsilon, activation_func=activation_func
        )
        self.del_mixin("cls")
        self.add_mixin(
            "image_enc",
            ImageMixin(args.hidden_size, args.projection_dim, layernorm_epsilon),
        )
        self.del_mixin("patch_embedding")
        self.add_mixin(
            "patch_embedding",
            PatchMixin(args.in_channels, args.hidden_size, self.property),
        )

    @classmethod
    def add_model_specific_args(cls, parser):
        group = parser.add_argument_group(
            "CLIP-image", "CLIP image encoder Configurations"
        )
...
class CLIP(nn.Module):
    ...
    def forward(
        ...
        **kw_args
        ...
        image_embeds, *image_mems = self.encode_image(
            image_input_ids,
            image_position_ids,
            attention_mask=image_attention_mask,
            **kw_args
        )
        text_embeds, *text_mems = self.encode_text(
            text_input_ids,
            text_position_ids,
    ...
    def add_model_specific_args(cls, parser):
        ...
        group.add_argument(
            "--text-hidden-size-per-attention-head", type=int, default=None
        )
    ...
    def from_pretrained(cls, args, name, *, path=None, url=None):
        ...
        args = update_args_with_file(
            args, path=os.path.join(model_path, "model_config.json")
        )
