...
class ImagePatchEmbeddingMixin(BaseMixin):
    def __init__(self, in_channels, hidden_size, property):
        ...
        self.proj = nn.Conv2d(
            in_channels,
            hidden_size,
            kernel_size=property.patch_size,
            stride=property.patch_size,
        )
    ...
    def word_embedding_forward(self, input_ids, **kwargs):
        ...
        * (batch_size, hidden_size)
        ...
        pre_word_embeddings = self.transformer.word_embeddings(
            input_ids[:, : self.transformer.property.pre_len]
        )
        post_word_embeddings = self.transformer.word_embeddings(
            input_ids[
                :,
                self.transformer.property.pre_len : self.transformer.property.pre_len
                + self.transformer.property.post_len,
            ]
        )
        embeddings = torch.cat(
            [pre_word_embeddings, embeddings, post_word_embeddings], dim=1
        )
        return embeddings
...
class InterpolatedPositionEmbeddingMixin(BaseMixin):
    ...
    def reinit(self, parent_model=None, property=None):
        ...
        post_weight = old_weight[
            self.transformer.property.pre_len + self.transformer.property.num_patches :
        ]
        image_weight = (
            old_weight[
                self.transformer.property.pre_len : self.transformer.property.pre_len
                + self.transformer.property.num_patches
            ]
            .reshape(
                1,
                self.transformer.property.grid_size[0],
                self.transformer.property.grid_size[1],
                -1,
            )
            .permute(0, 3, 1, 2)
        )
        ...
        image_weight = (
            F.interpolate(
                image_weight.float(),
                size=property.grid_size,
                mode="bicubic",
                align_corners=False,
            )
            .permute(0, 2, 3, 1)
            .reshape(property.num_patches, -1)
            .to(original_dtype)
        )
        self.transformer.position_embeddings = (
            torch.nn.Embedding(property.seq_len, old_weight.shape[1])
            .type(old_weight.dtype)
            .to(old_weight.device)
        )
        torch.nn.init.normal_(
            self.transformer.position_embeddings.weight, mean=0.0, std=0.02
        )
        self.transformer.position_embeddings.weight.data[
            : self.transformer.property.pre_len
        ] = pre_weight
        self.transformer.position_embeddings.weight.data[
            property.pre_len : property.pre_len + property.num_patches
        ] = image_weight
        self.transformer.position_embeddings.weight.data[
            property.pre_len
            + property.num_patches : property.pre_len
...
class ViTModel(BaseModel):
    def __init__(self, args, transformer=None, **kwargs):
        property = ViTProperty(
            args.image_size, args.patch_size, args.pre_len, args.post_len
        )
        args.max_sequence_length = (
            property.pre_len + property.num_patches + property.post_len
        )
        ...
        self.add_mixin(
            "patch_embedding",
            ImagePatchEmbeddingMixin(args.in_channels, args.hidden_size, property),
        )
    ...
    def add_model_specific_args(cls, parser):
        ...
        group.add_argument(
            "--post-len", type=int, default=0
        )  # empty by default, but sometimes with special tokens, such as [det] in yolos.
