...
class NewTokenMixin(ImagePatchEmbeddingMixin):
    def __init__(
        self, new_token_size, in_channels, hidden_size, property, init_method_std=0.02
    ):
    ...
    def reinit(self, parent_model=None):
        ...
        self.transformer.word_embeddings = (
            torch.nn.Embedding(self.new_token_size, old_weight.shape[1])
            .type(old_weight.dtype)
            .to(old_weight.device)
        )
        torch.nn.init.normal_(
            self.transformer.word_embeddings.weight, mean=0.0, std=self.init_method_std
        )
...
class DetHeadMixin(BaseMixin):
    def __init__(self, args):
        ...
        self.class_embed = MLP(
            args.hidden_size, args.hidden_size, args.num_det_classes, 3
        )
...
class YOLOS(ViTModel):
    def __init__(self, args, transformer=None, layernorm_epsilon=1e-6, **kwargs):
        ...
        self.add_mixin(
            "patch_embedding",
            NewTokenMixin(
                args.vocab_size + args.num_det_tokens,
                args.in_channels,
                args.hidden_size,
                self.property,
            ),
