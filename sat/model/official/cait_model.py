...
from sat.model.official.vit_model import ClsMixin, ViTModel
...
class AttnMixin(BaseMixin):
    def __init__(self, num_heads, num_layers):
        ...
        self.proj_l = nn.ModuleList(
            [nn.Linear(num_heads, num_heads) for i in range(num_layers)]
        )
        ...
        **kwargs
        ...
        if scaling_attention_score:
            query_layer = query_layer * (
                query_layer.shape[-1] ** -0.5
            )  # / math.sqrt(query_layer.shape[-1])
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))
        if log_attention_weights is not None:
            attention_scores += log_attention_weights

        attention_scores = self.proj_l[kwargs["layer_id"]](
        ...
        attention_probs = self.proj_w[kwargs["layer_id"]](
...
class EncForward(BaseMixin):
    ...
    def layer_forward(self, hidden_states, mask, *args, **kw_args):
        ...
        # Residual connection.
        layernorm_input = (
            hidden_states + self.gamma_1[kw_args["layer_id"]] * attention_output
        )
...
class DecForward(BaseMixin):
    ...
    def cross_attention_forward(
        ...
        new_context_layer_shape = context_layer.size()[:-2] + (
            layer.hidden_size_per_partition,
        )
        # [b, s, hp]
...
class CaiTEncoder(ViTModel):
    def __init__(
        self, args, transformer=None, layernorm_epsilon=1e-6, use_final_layernorm=False
    ):
        super().__init__(
            args,
            transformer=transformer,
            layernorm_epsilon=layernorm_epsilon,
            use_final_layernorm=use_final_layernorm,
        )
        self.del_mixin("cls")
        self.add_mixin("attn", AttnMixin(args.num_attention_heads, args.num_layers))
        self.add_mixin(
            "enc_forward",
            EncForward(args.hidden_size, args.num_layers, init_values=args.init_scale),
        )
...
class CaiTDecoder(BaseModel):
    def __init__(self, args, transformer=None, layernorm_epsilon=1e-6):
        ...
        self.add_mixin(
            "dec_forward",
            DecForward(args.hidden_size, args.num_layers, init_values=args.init_scale),
        )
...
class CaiT(EncoderDecoderModel):
    ...
    def forward(
        ...
        **kw_args
        ...
        decoder_outputs, *mems = self.decode(
            input_ids,
            dec_position_ids,
            dec_attention_mask,
