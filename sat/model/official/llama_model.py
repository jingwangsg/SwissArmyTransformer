...
class RotaryMixin(BaseMixin):
    ...
    def attention_forward(self, hidden_states, mask, **kw_args):
        ...
        query_layer, key_layer = origin.rotary_emb(
            query_layer,
            key_layer,
...
class LLaMAMlpMixin(BaseMixin):
    def __init__(self, num_layers, in_features, hidden_features):
        super().__init__()
        hidden_features = (
            4 * in_features if hidden_features is None else hidden_features
        )
    ...
    def mlp_forward(self, hidden_states, **kw_args):
        ...
        hidden_states = origin.activation_func(
            self.gate_proj[kw_args["layer_id"]](hidden_states)
        ) * origin.dense_h_to_4h(hidden_states)
...
class LLaMAModel(BaseModel):
    def __init__(
        ...
        transformer=None,
        layernorm=RMSNorm,
        activation_func=nn.functional.silu,
        **kwargs
    ):
        super().__init__(
            args,
        ...
        if not (hasattr(args, "is_rotary_emb") and args.is_rotary_emb):
            ...
            self.add_mixin(
                "rotary", RotaryMixin(args.hidden_size, args.num_attention_heads)
            )
