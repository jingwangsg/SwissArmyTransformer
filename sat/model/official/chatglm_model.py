...
from sat.model.position_embedding.rotary_embeddings import (
    RotaryEmbedding, apply_rotary_pos_emb_index)
...
class ChatGLMModel(BaseModel):
    def __init__(self, args, transformer=None, **kwargs):
        super(ChatGLMModel, self).__init__(
            args, transformer=transformer, activation_func=gelu, **kwargs
        )
        del self.transformer.position_embeddings
        self.add_mixin(
            "chatglm-final", ChatGLMFinalMixin(args.vocab_size, args.hidden_size)
        )
        self.add_mixin(
            "chatglm-attn", ChatGLMAttnMixin(args.hidden_size, args.num_attention_heads)
        )
    ...
    def forward(
        ...
        **kwargs
        ...
        if attention_mask is None and position_ids is None:
            attention_mask, position_ids = self.get_inputs(
                input_ids,
        ...
        if attention_mask is None:
            if past_key_values is not None and input_ids.size(0) == 1:
                ...
                attention_mask = self.get_masks(
                    input_ids=input_ids, device=input_ids.device, **kwargs
                )
    ...
    def get_position_ids(
        self, input_ids, mask_positions, device, gmask=False, **kwargs
    ):
        batch_size, seq_length = input_ids.shape
        pad_lengths = [self.get_pad_length(seq.tolist()) for seq in input_ids]
        context_lengths = [seq.tolist().index(self.bos_token_id) for seq in input_ids]
        position_ids = [
            torch.arange(seq_length - pad_length, dtype=torch.long, device=device)
            for pad_length in pad_lengths
        ]
        for i, (context_length, pad_length) in enumerate(
            zip(context_lengths, pad_lengths)
        ):
            position_ids[i][context_length - pad_length :] = (
                mask_positions[i] - pad_length
            )
        block_position_ids = [
            torch.cat(
                (
        ...
        position_ids = [
            torch.cat(
                (torch.zeros(pad_length, dtype=torch.long, device=device), range_pos)
            )
