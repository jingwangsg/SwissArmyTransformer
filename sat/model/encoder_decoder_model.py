...
from .base_model import BaseMixin, BaseModel, get_model, load_checkpoint
...
class EncoderDecoderModel(torch.nn.Module):
    def __init__(
        self, args, encoder=None, decoder=None, tie_word_embeddings=True, **kwargs
    ):
        ...
        if decoder is not None:
            ...
            override_attrs = [
                ...
                "hidden_size",
                "num_attention_heads",
                "layernorm_order" "max_sequence_length",
                "inner_hidden_size",
        ...
        if tie_word_embeddings:
            self.decoder.transformer.word_embeddings = (
                self.encoder.transformer.word_embeddings
            )

    def reinit(
        self, mixin_names
    ):  # please use different mixin names for encoder and decoder
    ...
    def encode(self, input_ids, position_ids, attention_mask=None, **kw_args):
        encoder_outputs, *_dumps = self.encoder(
            input_ids, position_ids, attention_mask, **kw_args
        )
        ...
        **kw_args
        ...
        if attention_mask is None:
            ...
            attention_mask = attention_mask.to(
                self.decoder.transformer.word_embeddings.weight.dtype
            )
    ...
    def forward(
        ...
        **kw_args
        ...
        encoder_outputs = self.encode(
            enc_input_ids, enc_position_ids, enc_attention_mask, **kw_args
        )
    ...
    def add_model_specific_args(cls, parser):
        ...
        group.add_argument(
            "--dec-hidden-size-per-attention-head", type=int, default=None
        )
    ...
    @classmethod
    def from_pretrained(
        cls, args, name, *, home_path=None, url=None
    ):  # TODO update model-only mode
