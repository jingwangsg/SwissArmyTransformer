...
from sat.mpu import (ColumnParallelLinear, RowParallelLinear,
                     VocabParallelEmbedding, checkpoint,
...
class BaseTransformerLayer(torch.nn.Module):
    def __init__(
        ...
        self.layer_id = layer_id
        self.is_decoder = (
            is_decoder[layer_id] if type(is_decoder) is list else is_decoder
        )
...
class BaseTransformer(torch.nn.Module):
    def __init__(
        ...
        ), f"checkpoint_skip_layers too large. Please consider remove checkpoint_activations."
        ...
        if vocab_size < 1000:
            self.word_embeddings = torch.nn.Embedding(
                vocab_size, hidden_size, dtype=params_dtype, device=device
            )
            torch.nn.init.normal_(
                self.word_embeddings.weight, mean=0.0, std=init_method_std
            )
        else:
        ...
        if self.is_rotary_emb:
            ...
            self.position_embeddings = FastRotaryEmbedding(
                hidden_size // num_attention_heads
            )
        else:
        ...
        **kw_args,
        ...
        if attention_mask is None:
            ...
            attention_mask = torch.ones(
                query_length, query_length, device=input_ids.device
            ).tril()
