...
def attention_fn_default(
    ...
    **kwargs,
    ...
    key_layer = (
        ...
        .expand(-1, -1, num_query_heads // num_kv_heads, -1, -1)
        .contiguous()
        .view(batch_size, num_query_heads, *key_layer.shape[2:])
    )
    value_layer = (
        value_layer.unsqueeze(2)
        .expand(-1, -1, num_query_heads // num_kv_heads, -1, -1)
        .contiguous()
    ...
    is_low_triangle = (
        attention_mask == torch.ones_like(attention_mask, dtype=torch.float).tril()
    ).all()
...
def mlp_forward_default(self, hidden_states, expert_id=-1, **kw_args):
    if self.transformer.num_experts == 1 or expert_id > -1:
        ...
        if self.is_gated_mlp:
            intermediate_parallel = getattr(self, "dense_h_to_4h" + suffix)(
                hidden_states
            )
            gated_intermediate_parallel = getattr(self, "dense_h_to_4h_gate" + suffix)(
                hidden_states
            )
            intermediate_parallel = (
                self.activation_func(gated_intermediate_parallel)
                * intermediate_parallel
            )
        ...
        routing_forward = self.hooks.get(
            "routing_forward", partial(routing_forward_default, self)
        )
        self = self.transformer.layers[kw_args["layer_id"]].mlp
        ...
        expert_mask = torch.nn.functional.one_hot(
            fwd_idx, num_classes=self.num_experts
        ).permute(2, 1, 0)
        ...
        for expert_idx in range(self.num_experts):
            ...
            current_state = hidden_states[
                top_x_list
            ]  # I don't know why using hidden_states[None, top_x_list].reshape(-1, hidden_dim)
            current_hidden_states = (
                mlp_forward(current_state, expert_id=expert_idx, **kw_args)
                * fwd_weight[top_x_list, idx_list, None]
            )
...
def layer_forward_default(self, hidden_states, mask, *args, **kw_args):
    ...
    if self.training and self.drop_path > 0.0:
        random_tensor = (
            ...
            - self.drop_path
            + torch.rand(
                (mlp_output.shape[0],), dtype=mlp_output.dtype, device=mlp_output.device
            )
        ).floor_() / (1 - self.drop_path)
