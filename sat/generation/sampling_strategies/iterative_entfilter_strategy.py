...
class IterativeEntfilterStrategy:
    ...
    def forward(
        ...
        if entfilter is not None:
            ...
            temperature = (
                torch.tensor(
                    [[[temperature - temperature2]]], device=logits.device
                ).expand(*logits.shape[:2], 1)
                * (ent > entfilter).unsqueeze(-1)
                + temperature2
            )
        ...
        pred = torch.multinomial(probs.view(-1, logits.shape[-1]), num_samples=1).view(
            *logits.shape[:2], 1
        )
        edge_idx = tk_idx[:, :, -1:]
        edge_value = tk_value[:, :, -1:]
        edge_mask = probs.gather(dim=-1, index=pred) < edge_value
        pred[edge_mask] = edge_idx[
            edge_mask
        ]  # replace outliers as the "filter_topk"-th token
