...
from sat.mpu import (get_data_parallel_rank, get_data_parallel_world_size,
                     get_model_parallel_rank)


def timed_name(prefix, suffix=None, path=None):
    return os.path.join(
        path, f"{prefix}-{datetime.now().strftime('%m-%d-%H-%M-%S')}{suffix}"
    )
...
def save_multiple_images(imgs, path, debug=True):
    ...
    if debug:
        ...
        save_image(
            torch.cat(imgs, dim=0), os.path.join(path, f"concat.jpg"), normalize=True
        )
        os.chmod(
            os.path.join(path, f"concat.jpg"),
            stat.S_IRWXO + stat.S_IRWXG + stat.S_IRWXU,
        )
