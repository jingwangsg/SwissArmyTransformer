...
def update_mems(hiddens, mems, max_memory_length):
    ...
    with torch.no_grad():
        if new_memory_length <= query_length:
            ...
            return torch.cat(
                (mems[:, :, -new_memory_length + query_length :], hiddens), dim=2
            )
    ...
    **kw_args,
    ...
    while counter < len(seq) - 1:
        ...
        if seq[counter + 1] >= 0:  # provided
            tokens = torch.cat(
                (tokens, seq[counter + 1 : counter + 2].expand(tokens.shape[0], 1)),
                dim=1,
            )
            counter += 1
            continue
...
def evaluate_perplexity(
    ...
    scores = -1 * torch.gather(pred, dim=2, index=target).squeeze(
        -1
