...
def make_data_loader(dataset, batch_size, args, split, collate_fn=None):
    ...
    if isinstance(dataset, IterableDataset):
        if split in ["val", "test"] and args.strict_eval:
            raise ValueError(
                "IterableDataset cannot be used for validation or testing if `args.strict_eval=True`, because we cannot infer the length of the final batch before reading out them."
            )
        ...
        per_rank_batch_size = (
            None if args.iterable_dataset == "custom" else batch_size // world_size
        )
    ...
    )  # some processes get full batch
    if last_len != 0:
        if last_len % batch_per_worker != 0:
            last_shape.append(
                last_len % batch_per_worker
            )  # one process get the rest (<1 batch)
...
def make_dataset_full(
    ...
    **kwargs,
    ...
    if (is_train_data and args.iterable_dataset) or (
        not is_train_data and args.iterable_dataset_eval
    ):  # cannot indexed
        ...
        if batch_from_same_dataset:
            assert (
                args.num_workers <= 1
            ), "We cannot control the actual speed of different workers, may mix different iterable parts."
        ds = AlterDataset(
            ds,
            weights=dataset_weights,
            seed=args.seed,
            batch_from_same_dataset=batch_from_same_dataset,
    ...
    if not should_split(split):
        ...
        if random_mapping:
            if (
            ...
            ):  # not auto-scale, but use a given number of epoches.
                ...
                if is_train_data:
                    ...
                    scale = max(
                        ...
                        1
                        + (
                            (1 + args.train_iters // args.eval_interval)
                            * args.eval_iters
        ...
        for p in path:
            ...
            if should_split(split):
                dtrain, dvalid, dtest = split_ds(
                    d, split, block_size=args.block_size, seed=args.seed
                )
        ...
        if random_mapping:
            ...
            scale = max(
                200,
                1 + (args.train_iters * args.batch_size * world_size) // len(train_ds),
            )
...
def make_loaders(args, create_dataset_function, collate_fn=None):
    ...
    make_dataset = partial(
        ...
        batch_from_same_dataset=args.batch_from_same_dataset,
    )
    ...
    if args.train_data is not None:
        train = make_dataset(
            **data_set_args,
            args=args,
            dataset_weights=args.train_data_weights,
            is_train_data=True,
        )
        if should_split(split):
    ...
    if valid is None and args.valid_data is not None:
        ...
        valid = make_dataset(
            **eval_set_args, args=args, random_mapping=not args.strict_eval
        )
    ...
    if train is not None and args.batch_size > 0:
        train = make_data_loader(
            train, batch_size, args, split="train", collate_fn=collate_fn
        )
        args.do_train = True
    else:
        args.do_train = False
    ...
    if valid is not None:
        valid = make_data_loader(
            valid, eval_batch_size, args, split="val", collate_fn=collate_fn
        )
        args.do_valid = True
    else:
        args.do_valid = False
    if test is not None:
        test = make_data_loader(
            test, eval_batch_size, args, split="test", collate_fn=collate_fn
        )
        args.do_test = True
    else:
        args.do_test = False
...
def split_ds(ds, split=[0.8, 0.2, 0.0], block_size=10000, seed=131):
    """
    ...
    for i, f in enumerate(split):
        if f != 0:
            ...
            rtn_ds[i] = BlockedRandomSplitDataset(
                ds, indices[range(start_idx, start_idx + max(split_, 1))], block_size
            )
            start_idx += split_
            residual_idx %= 1
    return rtn_ds
