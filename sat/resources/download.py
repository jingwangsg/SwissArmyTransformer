    ...
    with lock:
        ...
        if os.path.isdir(model_path) and not url.startswith("r2://"):
            ...
            try:
                ...
                print(
                    f"Failed to download or check, if you already had the zip file, please unzip it manually as {model_path}!"
                )
                raise e
        # unzip
...
def download_s3(local_dir, remote_uri):
    ...
    if "." in os.path.basename(remote_uri):
        bucket.download_file(
            remote_uri,
            os.path.join(local_dir, os.path.basename(remote_uri)),
            Callback=ProgressPercentage(client, SAT_BUCKET, remote_uri),
    ...
    for obj in bucket.objects.filter(Prefix=remote_dir):
        ...
        # skip if exists
        if (
            os.path.exists(os.path.join(local_dir, key_suffix))
            and os.path.getsize(os.path.join(local_dir, key_suffix)) == obj.size
        ):
            continue
        bucket.download_file(
            obj.key,
            os.path.join(local_dir, key_suffix),
            Callback=ProgressPercentage(client, SAT_BUCKET, obj.key),
            Config=transfer_config,
        )
    ...
    def __call__(self, bytes_amount):
        ...
        with self._lock:
            ...
            output = (
                ...
                + str(percentage)
                + "% "
                + self.convert_bytes(self._seen_so_far)
                + " / "
                + self.convert_bytes(self._size)
                + " " * 5
            )
