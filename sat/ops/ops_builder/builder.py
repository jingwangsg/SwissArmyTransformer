...
try:
    ...
    print(
        f"{WARNING} unable to import torch, please install it if you want to pre-compile any sat ops."
    )
else:
...
# to build sat and system-wide installed cuda 11.2
...
def assert_no_cuda_mismatch(name=""):
    ...
    if sys_cuda_version != torch_cuda_version:
        if (
            ...
            return True
        raise Exception(
            f">- sat Op Builder: Installed CUDA version {sys_cuda_version} does not match the "
...
class OpBuilder(ABC):
    ...
    def validate_torch_version(torch_info):
        ...
        if install_torch_version != current_torch_version:
            raise RuntimeError(
                "PyTorch version mismatch! SAT ops were compiled and installed "
                "with a different version than what is being used at runtime. "
                f"Please re-install SAT or switch torch versions. "
    ...
    def validate_torch_op_version(torch_info):
        if not OpBuilder.is_rocm_pytorch():
            ...
            if install_cuda_version != current_cuda_version:
                raise RuntimeError(
                    "CUDA version mismatch! SAT ops were compiled and installed "
                    "with a different version than what is being used at runtime. "
                    f"Please re-install SAT or switch torch versions. "
            ...
            if install_hip_version != current_hip_version:
                raise RuntimeError(
                    "HIP version mismatch! SAT ops were compiled and installed "
                    "with a different version than what is being used at runtime. "
                    f"Please re-install SAT or switch torch versions. "
    ...
    def sat_src_path(self, code_path):
