...
def add_model_config_args(parser):
    ...
    group.add_argument(
        "--hidden-size", type=int, default=1024, help="transformer hidden size"
    )
    group.add_argument(
        "--num-attention-heads",
        type=int,
    ...
    group.add_argument(
        ...
        choices=[
            "post",  # In the original Transformer.
    ...
    group.add_argument(
        "--skip-init", action="store_true", help="skip model initialization"
    )

    group.add_argument(
        "--use-gpu-initialization", action="store_true", help="initialize model on gpu"
    )
    ...
    group.add_argument(
        "--drop-path", type=float, default=0.0, help="drop path probability"
    )
    group.add_argument(
    ...
    group.add_argument(
        "--parallel-output",
        action="store_true",
        help="whether to gather model parallel outputs at final output. Need to be True if using mpu.vocab_parallel_cross_entropy.",
    )
...
def add_training_args(parser):
    ...
    group.add_argument("--bf16", action="store_true", help="Run model in bf16 mode")
    group.add_argument(
    ...
    group.add_argument("--log-interval", type=int, default=50, help="report interval")
    group.add_argument(
        "--summary-dir", type=str, default="", help="The directory to store the summary"
    )
    group.add_argument(
        "--save-args",
        action="store_true",
...
def add_data_args(parser):
    ...
    parser.add_argument(
        ...
        help="custom means Dataloader batch_size=None, and you need to generate batches by yourself. This is useful for dynamic batchsize.",
    )
    parser.add_argument(
        "--iterable-dataset-eval",
        default="",
        type=str,
        choices=["True", "False", ""],
        help="Whether eval is iterable.",
    )

    group.add_argument(
        "--batch-from-same-dataset",
        action="store_true",
    ...
    group.add_argument(
        "--test-data", nargs="*", default=None, help="""Filename for testing"""
    )
    group.add_argument(
        "--split",
        default="1000,1,1",
    ...
    group.add_argument(
        "--prefetch-factor", type=int, default=4, help="prefetching number of batches."
    )
    return parser
...
def add_text_generate_args(parser):
    ...
    group.add_argument(
        "--with-id", action="store_true", help="If each line is prepended with an id."
    )
...
def add_tokenization_args(parser):
    ...
    group.add_argument(
        "--tokenizer-type", type=str, default="fake", help="type name of tokenizer"
    )
...
def get_args(args_list=None, parser=None):
    ...
    if args.train_data_weights is not None:
        assert len(args.train_data_weights) == len(
            args.train_data
        ), f"{args.train_data_weights} vs {args.train_data}"
...
def reset_random_seed(scale=1):
    assert (
        _GLOBAL_RANDOM_SEED is not None
    ), "You have not set random seed. No need to reset it."
