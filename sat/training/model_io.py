...
def update_ema_parameters_to_model(optimizer):
    ...
    if version.parse(deepspeed.version) >= version.parse("0.12.4"):
        all_gather_dp_groups(
            groups_flat=optimizer.bit16_groups_flat,
...
def save_checkpoint(iteration, model, optimizer, lr_scheduler, args):
    ...
    if hasattr(args, "deepspeed") and args.deepspeed:
        ...
        if (
            ...
            and optimizer.optimizer.__class__.__name__ == "FusedEmaAdam"
        ):
...
def save_ds_checkpoint(iteration, model, lr_scheduler, args, use_ema=False):
    ...
    if not use_ema:
        ...
        save_ds_checkpoint_no_optim(
            model, args.save, str(iteration) + "-ema", client_state=sd
        )
...
def save_ds_checkpoint_no_optim(
    ...
    if model.zero_optimization_partition_weights():
        ...
        torch.save(
            {"module": zero3_state_dict(model)},
            os.path.join(
                save_dir,
                tag,
                "z3_rank_{:02d}_model_states.pt".format(torch.distributed.get_rank()),
            ),
        )
    else:
    ...
    if len(missing_keys) > 0:
        if args.mode == "inference":
            ...
            if not args.force_train:
                assert all(
                    name.find("mixins") >= 0 or name.find("cross_attention") >= 0
...
def zero3_save_to_state_dict(module, destination, prefix, keep_vars):
    ...
    for name, param in module._parameters.items():
        if param is not None:
            if hasattr(param, "ds_id"):
                ...
                destination[prefix + name].ds_shape = (
                    param.ds_shape
                )  # record the unpartitioned shape
            else:
    ...
    if (
        getattr(module.__class__, "get_extra_state", torch.nn.Module.get_extra_state)
