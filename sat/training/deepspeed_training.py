...
from .utils import (Timers, get_sample_writer, init_wandb_writer, print_args,
                    report_memory)
...
def training_main(
    ...
    if args.load and args.mode == "pretrain":  # continue training
        ...
        args.experiment_name = (
            args.experiment_name + "-" + datetime.now().strftime("%m-%d-%H-%M")
        )
    ...
    # Data stuff.
    train_data, val_data, test_data = make_loaders(
        args, hooks["create_dataset_function"], collate_fn=collate_fn
    )
    if args.epochs:
        ...
        if args.do_train:
            with ExitStack() as stack:
                ...
                def save_on_exit(args_, model_, optimizer_, lr_scheduler_):
                    save_checkpoint(
                        args_.iteration, model_, optimizer_, lr_scheduler_, args_
                    )
    ...
    if args.do_test and test_data is not None:
        ...
        test_loss = evaluate_and_print_results(
            prefix,
            iter(test_data),
            model,
            len(test_data) if args.strict_eval else args.eval_iters,
            args,
            timers,
            True,
            split="test",
    ...
    if args.train_data is not None:
        if args.deepspeed:
            ...
            if optimizer_name.startswith("sat."):
                ...
                # split and import
                optimizer_callable = getattr(
                    import_module(optimizer_name.rsplit(".", maxsplit=1)[0]),
                    optimizer_name.split(".")[-1],
                )
                optimizer_callable = partial(
                    optimizer_callable,
                    **args.deepspeed_config.get("optimizer", {}).get("params", {}),
                )
...
def add_param_by_lr(dic, p, no_weight_decay=False):
    if not hasattr(p, "lr_scale"):
        ...
        if p.lr_scale not in dic:
            dic[p.lr_scale] = (
                {"params": [], "lr": p.lr_scale}
                if not no_weight_decay
                else {"params": [], "weight_decay": 0.0, "lr": p.lr_scale}
            )
        dic[p.lr_scale]["params"].append(p)


def get_params_for_weight_decay_optimization(module):
    weight_decay_params = {None: {"params": [], "lr": 1.0}}
    no_weight_decay_params = {None: {"params": [], "weight_decay": 0.0, "lr": 1.0}}
        ...
        if isinstance(module_, tuple(NO_WD_MODULES)):
            ...
            for n, p in module_._parameters.items():
                if p is not None and n != "bias" and p.requires_grad:
                    flag = (
                        True
                        if hasattr(p, "no_weight_decay") and p.no_weight_decay
                        else False
                    )
                    if flag:
...
def get_optimizer_param_groups(model):
    ...
    for param_group in param_groups:
        for param in param_group["params"]:
            if not hasattr(param, "model_parallel") and not hasattr(
                param, "tensor_model_parallel"
            ):
                param.model_parallel = False
                param.tensor_model_parallel = False
            else:
                assert hasattr(param, "model_parallel") and hasattr(
                    param, "tensor_model_parallel"
                ), "model_parallel and tensor_model_parallel should both be set or unset!"
    return param_groups


def get_learning_rate_scheduler(
    optimizer, iteration, args, auto_warmup_steps=100, auto_warmup_rate=0.05
):
...
def train(
    ...
    while args.iteration < args.train_iters:
        ...
        lm_loss, skipped_iter, metrics = train_step(
            train_data_iterator,
            model,
            optimizer,
            lr_scheduler,
        ...
        for name in metrics:
            if not "eval" in name:
                ...
                ), "metrics without eval must be scalar"
        ...
        if args.iteration % args.log_interval == 0:
            ...
            for key in total_metrics:
                avg_metrics[key] = (
                    total_metrics[key] / total_metrics_cnt[key]
                )  # args.log_interval
        ...
        if (
            args.eval_interval
            and args.iteration % args.eval_interval == 0
            and args.do_valid
        ):
            if args.strict_eval:
                val_data_iterator = iter(val_data)
...
def train_step(
    ...
    **kwargs,
    ...
    while True:
        ...
        for name in metrics:
            if not "eval" in name:
                ...
                if metrics[name].data.item() == -100:
                    cnt = torch.zeros(
                        1, dtype=torch.int64, device=metrics[name].data.device
                    )
                    metrics[name].data = torch.tensor(
                        0.0, device=metrics[name].data.device
                    )
                else:
                    cnt = torch.ones(
                        1, dtype=torch.int64, device=metrics[name].data.device
                ...
                if cnt.item() == 0:
                    metrics[name].data = torch.tensor(
                        -100, device=metrics[name].data.device
                    )
                else:
        ...
        for name in metrics:
            if name not in metrics_total:
                metrics_total[name] = torch.tensor(
                    0.0, device=metrics[name].data.device
                )
    ...
    metrics_total = {
        key: (
            torch.tensor(-100, device=metrics_total[key].data.device)
            if metrics_count[key] == 0
            else value / metrics_count[key]
        )
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        while iteration < eval_iters:
            ...
            is_last = (
                True
                if iteration == eval_iters and args.strict_eval and len(last_shape) > 0
                else False
            )
            for name in metrics:
                if name not in metrics_total:
                ...
                if (
                    not is_scalar[name]
                    and is_last
                    and metrics[name].shape[0] != last_shape[0]
                ):
                    # pad tensor's first dim to args.batch_size
                    metrics[name] = torch.concat(
                        [
                            metrics[name],
                            torch.zeros(
                                [last_shape[0] - metrics[name].shape[0]] + shape[1:],
                                dtype=metrics[name].dtype,
                ...
                if rank == 0:
                    gathered_len = (
                        ...
                        if not is_last
                        else len(metrics_gathered)
                        - drop_number * args.model_parallel_size
                    )
                    for i in range(gathered_len):
                        if is_scalar[name] or not is_last:
                            ...
                            metrics_total[name].append(
                                metrics_gathered[i][: last_shape[i]].data.cpu()
                            )
    ...
    if rank == 0:
        ...
        if hooks["handle_metrics"] is not None:
            ...
            for name in metrics_total:
                assert is_scalar[
                    name
                ], "you must return scalar metrics or implement handle_metrics hooks"
            metrics = {
                key: sum(value.split(1, 0)) / len(value)
                for key, value in metrics_total.items()
            }
    else:
        metrics = None
...
def evaluate_and_print_results(
    ...
    if torch.distributed.get_rank(group=mpu.get_data_parallel_group()) == 0:
        report_evaluate_metrics(
            summary_writer, prefix, lm_loss, lm_ppl, step, args, metrics
        )
    return lm_loss
