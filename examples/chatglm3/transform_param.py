...
chatglm = (
    AutoModel.from_pretrained("THUDM/" + model_type, trust_remote_code=True)
    .half()
    .cuda()
)
...
def copy_transformer_layer(src, dst, w2):
...
def transform_weight(hugging_model, swiss_model):
    ...
    for src_l, dst_l, w2 in zip(
        hugging_model.transformer.encoder.layers,
        swiss_model.transformer.layers,
        swiss_model.mixins["mlp"].w2,
    ):
        copy_transformer_layer(src_l, dst_l, w2)
    copy_layer_param(
...
with torch.no_grad():
    ...
    dst_output = model.cuda()(
        ...
        attention_mask=torch.ones(
            1, 1, 2, 2, dtype=torch.float16, device="cuda"
        ).tril(),
    )
