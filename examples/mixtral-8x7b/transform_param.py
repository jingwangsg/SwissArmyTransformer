...
hugging = AutoModelForCausalLM.from_pretrained(
...
def copy_layer_param(src, dst):
    ...
    for k in dst_dic:
        assert (
            dst_dic[k].data.shape == src_dic[k].data.shape
        ), f"src {src_dic[k].data.shape}, dst {dst_dic[k].data.shape}"
...
def copy_transformer_layer(src, dst, gate):
    new_weight = torch.cat(
        [
            src.self_attn.q_proj.weight.data,
            src.self_attn.k_proj.weight.data,
            src.self_attn.v_proj.weight.data,
        ],
        0,
    )
    ...
    for i in range(len(src.block_sparse_moe.experts)):
        ...
        copy_layer_param(
            src.block_sparse_moe.experts[i].w1,
            getattr(dst.mlp, "dense_h_to_4h_gate" + suffix),
        )
        copy_layer_param(
            src.block_sparse_moe.experts[i].w3,
            getattr(dst.mlp, "dense_h_to_4h" + suffix),
        )
        copy_layer_param(
            src.block_sparse_moe.experts[i].w2,
            getattr(dst.mlp, "dense_4h_to_h" + suffix),
        )
...
def transform_weight(hugging_model, swiss_model):
    copy_layer_param(
        hugging_model.model.embed_tokens, swiss_model.transformer.word_embeddings
    )
    copy_layer_param(hugging_model.lm_head, swiss_model.get_mixin("lm").lm_head)
    for src_l, dst_l, gate in zip(
        hugging_model.model.layers,
        swiss_model.transformer.layers,
        swiss_model.get_mixin("mlp").gates,
    ):
...
with torch.no_grad():
    ...
    batch = tokenizer(
        "The capital of China is", return_tensors="pt", add_special_tokens=False
    )
    ...
    batch["attention_mask"] = (
        batch["attention_mask"]
        .unsqueeze(1)
        .repeat_interleave(batch["attention_mask"].shape[-1], 1)
        .tril()
        .unsqueeze(1)
    )
    out_s = model(**batch)
