...
if model_type == "tiny":
    from config.deit_tiny_config import args, vit
elif model_type == "small":
    from config.deit_small_config import args, vit
elif model_type == "base":
    from config.deit_base_config import args, vit
else:
    raise Exception(
        "Unknown model type. You may see https://github.com/facebookresearch/deit/blob/main/models.py for more model configs"
    )
...
def transform_weight(src_model, swiss_model):
    copy_from_param(
        src_model.cls_token.data[0], swiss_model.transformer.word_embeddings.weight
    )
    copy_from_param(
        src_model.pos_embed.data[0], swiss_model.transformer.position_embeddings.weight
    )
    ...
    copy_layer_param(
        src_model.patch_embed.proj, swiss_model.mixins.patch_embedding.proj
    )
...
with torch.no_grad():
    ...
    encoded_input = {
        "input_ids": torch.zeros(2, 197).long(),
        "image": torch.randn(2, 3, 224, 224) * 10,
