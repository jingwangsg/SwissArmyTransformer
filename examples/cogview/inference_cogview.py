...
from sat.generation.utils import (generate_continually, save_multiple_images,
                                  timed_name)
...
def main(args):
    ...
    query_template = (
        "[ROI1] {} [BASE] [BOI1] [MASK]*1024" if not args.full_query else "{}"
    )
    invalid_slices = [slice(tokenizer.img_tokenizer.num_tokens, None)]
    strategy = BaseStrategy(
        invalid_slices, temperature=args.temperature, top_k=args.top_k
    )
    ...
    def process(raw_text):
        ...
        log_attention_weights[txt_len + 2 :, 1:txt_len] = (
            1.8 if txt_len <= 10 else 1.4
        )  # TODO args
        # generation
