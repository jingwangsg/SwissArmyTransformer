...
class FineTuneModel(ChatGLMModel):
    def __init__(self, args, transformer=None, **kw_args):
        ...
        if args.use_lora:
            self.add_mixin(
                "lora", LoraMixin(args.num_layers, args.lora_rank), reinit=True
            )

    @classmethod
    def add_model_specific_args(cls, parser):
        group = parser.add_argument_group(
            "ChatGLM-finetune", "ChatGLM finetune Configurations"
        )
...
def forward_step_eval(data_iterator, model, args, timers):
    ...
    tokens, labels = get_batch(data_iterator, args, timers)
    ...
    return torch.tensor(0, device=outputs.device), {
        k: torch.tensor(v, device=outputs.device)
        for k, v in compute_metrics((outputs.cpu(), labels.cpu())).items()
    }
...
def create_dataset_function(path, args):
    ...
    def preprocess_function_train(examples):
        ...
        for i in range(len(examples[args.prompt_column])):
            if examples[args.prompt_column][i] and examples[args.response_column][i]:
                prompt, answer = (
                    examples[args.prompt_column][i],
                    examples[args.response_column][i],
                )
