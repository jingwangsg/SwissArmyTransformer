...
def transform_weight(hugging_model, swiss_model):
    ...
    copy_layer_param(
        lm_head.predictions.transform.dense, model.mixins["bert-final"].lm_head.dense
    )
    copy_layer_param(
        lm_head.predictions.transform.LayerNorm,
        model.mixins["bert-final"].lm_head.layer_norm,
    )
    copy_layer_param(
        lm_head.predictions.decoder, model.mixins["bert-final"].lm_head.decoder
    )
    copy_layer_param(
        hugging_model.embeddings.token_type_embeddings,
        swiss_model.mixins["bert-type"].type_embeddings,
    )
...
with torch.no_grad():
    ...
    position_ids = (
        torch.arange(seq_len).unsqueeze(0).expand_as(encoded_input["input_ids"])
    )
    print(position_ids)
