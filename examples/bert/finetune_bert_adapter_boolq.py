...
from sat.model.mixins import AdapterMixin, MLPHeadMixin
...
class AdapterModel(BertModel):
    def __init__(self, args, transformer=None, layernorm_epsilon=1e-12, **kwargs):
        super().__init__(
            args, transformer=transformer, layernorm_epsilon=layernorm_epsilon, **kwargs
        )
        self.del_mixin("bert-final")
        self.add_mixin("classification_head", MLPHeadMixin(args.hidden_size, 2048, 1))
        self.add_mixin(
            "adapter",
            AdapterMixin(args.num_layers, args.hidden_size, args.adapter_hidden),
        )
    ...
    def add_model_specific_args(cls, parser):
        group = parser.add_argument_group(
            "BERT-finetune", "BERT finetune Configurations"
        )
...
if __name__ == "__main__":
    ...
    model, args = AdapterModel.from_pretrained(args.md_type, args)
