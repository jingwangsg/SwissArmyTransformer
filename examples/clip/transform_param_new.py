...
def copy_layer_param(src, dst):
    ...
    for k in dst_dic:
        assert dst_dic[k].data.shape == src_dic[k].data.shape, (
            k,
            dst_dic[k].data.shape,
            src_dic[k].data.shape,
        )
...
def transform_image_weight(src_model, swiss_model):
    ...
    copy_from_param(
        src_model.embeddings.class_embedding.unsqueeze(0).data,
        swiss_model.transformer.word_embeddings.weight,
    )
    copy_layer_param(
        src_model.embeddings.position_embedding,
    ...
    copy_layer_param(
        src_model.embeddings.patch_embedding, swiss_model.mixins.patch_embedding.proj
    )
...
def transform_text_weight(src_model, swiss_model):
    copy_layer_norm(src_model, swiss_model)
    copy_layer_param(
        src_model.embeddings.token_embedding, swiss_model.transformer.word_embeddings
    )
    copy_layer_param(
        src_model.embeddings.position_embedding,
        swiss_model.transformer.position_embeddings,
...
def transform_both_weight(src_model, swiss_model):
    ...
    copy_layer_param(
        src_model.visual_projection,
        swiss_model.image_encoder.mixins.image_enc.visual_projection,
    )
    copy_layer_param(
        src_model.text_projection,
        swiss_model.text_encoder.mixins.text_enc.text_projection,
    )
...
inputs = processor(
    text=["a photo of a monkey", "cat cat cat cat cat"],
    images=image,
    return_tensors="pt",
    padding=True,
)
expanded_mask = (
    inputs["attention_mask"][:, None, None, :].expand(2, 1, 7, 7).to(torch.float)
)
vit.eval()
model.eval()
with torch.no_grad():
    ...
    encoded_input = {
        "text_attention_mask": expanded_mask,
        "image_input_ids": torch.zeros(1, 1).long(),
    ...
    image_embeds, text_embeds, logits_per_text, logits_per_image = model(
        offline=True, **encoded_input
    )
