...
def get_batch(data_iterator, args, timers):
    ...
    processor = CLIPProcessor.from_pretrained(
        os.path.join(pretrain_path, "openai/clip-vit-base-patch32")
    )
    inputs = processor(
        text=[
            "airplane",
    ...
    text_position_ids = (
        torch.arange(inputs["input_ids"].size(1))
        .unsqueeze(0)
        .expand_as(inputs["input_ids"])
        .to(label_data.device)
    )
    text_attention_mask = (
        inputs["attention_mask"][:, None, None, :]
        .expand(
            text_input_ids.size(0), 1, text_input_ids.size(1), text_input_ids.size(1)
        )
        .to(label_data.device)
        .to(torch.float)
    )
