...
import torchvision
from datasets_.coco import CocoDetection, build
...
torch.distributed.init_process_group(
    backend="nccl",
    world_size=swiss_args.world_size,
    rank=swiss_args.rank,
...
swiss_model.get_mixin(
    "pos_embedding"
).reinit()  # patch_embedding should not reinit for inference
model = swiss_model
...
dataset = CocoDetection(
    img_folder,
    ann_file,
    transforms=make_coco_transforms(image_set, None),
    return_masks=False,
)
...
outputs = model(**encoded_input, offline=False, height=height // 16, width=width // 16)[
    0
]
