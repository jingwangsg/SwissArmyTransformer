...
import torch
from config.yolos_tiny_config import args, yolos
...
def transform_weight(src_model, swiss_model):
    words = torch.cat(
        (src_model.backbone.cls_token.data[0], src_model.backbone.det_token.data[0])
    )
    copy_from_param(words, swiss_model.transformer.word_embeddings.weight)
    copy_from_param(
        src_model.backbone.pos_embed.data[0],
    ...
    copy_layer_param(
        src_model.backbone.patch_embed.proj, swiss_model.mixins.patch_embedding.proj
    )
...
with torch.no_grad():
    ...
    encoded_input = {
        "input_ids": torch.cat(
            [torch.arange(1 + model.get_mixin("det_head").num_det_tokens)[None,]]
            * batch_size
        ).long(),
        "image": images,
    ...
    dst_output = model(
        **encoded_input, offline=True
    )  # offline=False, height=height//16, width=width//16)[0]
