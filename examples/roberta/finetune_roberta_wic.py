...
class ClassificationModel(RobertaModel):
    def __init__(self, args, transformer=None):
        ...
        self.add_mixin(
            "classification_head", WIC_MLPHeadMixin(args.hidden_size * 3, 2048, 1)
        )
        self.add_mixin(
            "prefix-tuning",
            PrefixTuningMixin(
...
def create_dataset_function(path, args):
    def process_fn(row):
        ...
        pos2 = (
            tokenizer(row["sentence2"][:start2])["input_ids"].__len__()
            - 2
            + tokenizer(row["sentence1"])["input_ids"].__len__()
        )
        return {
    ...
    return load_hf_dataset(
        ...
        offline=False,
        transformer_name="wic_transformer",
    )
