...
def forward_step(data_iterator, model, args, timers):
    ...
    true_pos = ((pred > 0.0).long() * labels).sum() * 1.0
    false_pos = ((1 - (pred > 0.0).long()) * labels).sum() * 1.0
    true_neg = ((1 - (pred > 0.0).long()) * (1 - labels)).sum() * 1.0
    false_neg = ((pred > 0.0).long() * (1 - labels)).sum() * 1.0
...
def create_dataset_function(path, args):
    ...
    return load_hf_dataset(
        path,
        process_fn,
        columns=["input_ids", "position_ids", "attention_mask", "label"],
        cache_dir="/dataset/fd5061f6/satDatasets",
        offline=True,
        transformer_name="boolq_transformer",
    )
