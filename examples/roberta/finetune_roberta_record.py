...
from sat.model.mixins import BaseMixin, MLPHeadMixin, PrefixTuningMixin
...
def get_batch(data_iterator, args, timers):
    ...
    padding_idx = 1
    pos_tokens_position_ids = create_position_ids_from_input_ids(
        pos_tokens.reshape([pos_tokens.shape[0] * pos_tokens.shape[1], -1]), 1, 0
    ).reshape(pos_tokens.shape)
    neg_tokens_position_ids = create_position_ids_from_input_ids(
        neg_tokens.reshape([neg_tokens.shape[0] * neg_tokens.shape[1], -1]), 1, 0
    ).reshape(neg_tokens.shape)
    pos_tokens_attention_mask = (
        torch.ones_like(pos_tokens, device=pos_tokens.device)
        * pos_tokens_position_ids.ne(padding_idx)
    )[:, :, None, None, :].float()
    neg_tokens_attention_mask = (
        torch.ones_like(neg_tokens, device=neg_tokens.device)
        * neg_tokens_position_ids.ne(padding_idx)
    )[:, :, None, None, :].float()
    ...
    return (
        pos_tokens,
        pos_tokens_position_ids,
        pos_tokens_attention_mask,
        neg_tokens,
        neg_tokens_position_ids,
        neg_tokens_attention_mask,
    )
...
def forward_step(data_iterator, model, args, timers):
    ...
    (
        ...
        neg_tokens_position_ids,
        neg_tokens_attention_mask,
    ) = get_batch(data_iterator, args, timers)
    ...
    for i in range(pos_tokens.shape[0]):
        ...
        for j in range(neg_tokens_now.shape[0]):
            if neg_tokens_now[j][0] == -1:
                ...
                neg_num = j
                break
        logits_pos, *mems = model(
            pos_tokens_now,
            pos_tokens_position_ids[i][:pos_num],
            pos_tokens_attention_mask[i][:pos_num],
        )
        logits_neg, *mems = model(
            neg_tokens_now,
            neg_tokens_position_ids[i][:neg_num],
            neg_tokens_attention_mask[i][:neg_num],
        )
        for j in range(pos_tokens_now.shape[0]):
            for k in range(neg_tokens_now.shape[0]):
                loss += F.margin_ranking_loss(
                    ...
                    logits_neg[k][0],
                    torch.ones_like(
                        logits_pos[j][0], device=pos_tokens_now.device
                    ).sign(),
                    1,
                )
...
tokenizer = RobertaTokenizer.from_pretrained(
    os.path.join(pretrain_path, "roberta-large")
)
...
def _encode(text, text_pair):
    ...
    position_ids = create_position_ids_from_input_ids(
        torch.tensor([encoded_input["input_ids"]]), 1, 0
    )
    return dict(
        input_ids=encoded_input["input_ids"],
        position_ids=position_ids[0].numpy(),
...
def create_dataset_function(path, args):
    ...
    return load_hf_dataset(
        path,
        process_fn,
        columns=["pos_tokens", "neg_tokens"],
        cache_dir="/dataset/fd5061f6/satDatasets",
        offline=True,
        transformer_name="record_transformer",
    )
