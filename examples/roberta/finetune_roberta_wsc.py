...
def get_batch(data_iterator, args, timers):
    ...
    cand_tokens = cand_tokens.reshape(
        [query_tokens.shape[0], -1, query_tokens.shape[1]]
    )
    ...
    query_attention_mask = (
        torch.ones_like(query_position_ids, device=query_tokens.device)
        * query_position_ids.ne(padding_idx)
    )[:, None, None, :].float()
    cand_tokens_position_ids = create_position_ids_from_input_ids(
        cand_tokens.reshape([cand_tokens.shape[0] * cand_tokens.shape[1], -1]), 1, 0
    ).reshape(cand_tokens.shape)
    cand_attention_mask = (
        torch.ones_like(cand_tokens_position_ids, device=query_tokens.device)
        * cand_tokens_position_ids.ne(padding_idx)
    )[:, :, None, None, :].float()
...
def create_dataset_function(path, args):
    ...
    return load_hf_dataset(
        path,
        process_fn,
        columns=["label", "query_tokens", "query_mask", "cand_tokens", "cand_mask"],
        cache_dir="/dataset/fd5061f6/satDatasets",
        offline=True,
        transformer_name="wsc_transformer",
    )
