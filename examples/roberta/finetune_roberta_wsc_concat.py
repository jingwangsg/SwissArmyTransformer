...
class ClassificationModel(RobertaModel):
    def __init__(self, args, transformer=None):
        ...
        self.add_mixin(
            "classification_head", WSC_MLPHeadMixin(args.hidden_size, 2048, 1)
        )
...
def forward_step(data_iterator, model, args, timers):
    ...
    tokens, labels, attention_mask, position_ids, loss_mask, start1, start2, end1 = (
        get_batch(data_iterator, args, timers)
    )
    timers("batch generator").stop()

    logits, *mems = model(
        tokens, position_ids, attention_mask, start1=start1, start2=start2, end1=end1
    )
    # pred = ((logits.contiguous().float().squeeze(-1)) * loss_mask).sum(dim=-1) / loss_mask.sum(dim=-1)
    pred = logits.contiguous().float().squeeze(-1)
    loss = torch.nn.functional.binary_cross_entropy_with_logits(pred, labels.float())
    acc = ((pred > 0.0).long() == labels).sum() / labels.numel()
    return loss, {"acc": acc}
