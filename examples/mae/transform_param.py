...
import torch
from config.mae_config import args, vit
...
def transform_weight(src_model, swiss_model):
    ...
    # transform encoder
    copy_from_param(
        src_model.cls_token.data[0],
        swiss_model.encoder.transformer.word_embeddings.weight,
    )
    copy_from_param(
        src_model.pos_embed.data[0],
        swiss_model.encoder.transformer.position_embeddings.weight,
    )
    for src_l, dst_l in zip(src_model.blocks, swiss_model.encoder.transformer.layers):
        copy_transformer_layer_wo_ln(src_l, dst_l)
    copy_layer_param(
        src_model.patch_embed.proj, swiss_model.encoder.mixins.patch_embedding.proj
    )
    # transform decoder
    copy_from_param(
        src_model.mask_token.data[0],
        swiss_model.decoder.transformer.word_embeddings.weight,
    )
    copy_from_param(
        src_model.decoder_pos_embed.data[0],
        swiss_model.decoder.transformer.position_embeddings.weight,
    )
    for src_l, dst_l in zip(
        src_model.decoder_blocks, swiss_model.decoder.transformer.layers
    ):
        copy_transformer_layer_wo_ln(src_l, dst_l)
    copy_layer_param(
        src_model.decoder_embed, swiss_model.decoder.mixins.mask_forward.decoder_embed
    )
    copy_layer_param(
        src_model.decoder_pred, swiss_model.decoder.mixins.mask_forward.decoder_pred
    )
...
with torch.no_grad():
    ...
    encoded_input = {
        "input_ids": torch.zeros(2, 1).long(),
        "image": torch.randn(2, 3, 224, 224),
        "enc_position_ids": position_ids,
        "dec_position_ids": dec_pos,
    }
    src_output = vit(encoded_input["image"], mask_ratio=0.0)[1]
    model = model.cuda()
    encoded_input = {k: v.cuda() for k, v in encoded_input.items()}
    dst_enc_output, dst_output, *_ = model(
        offline=True, mask_ratio=0.0, **encoded_input
    )
    ...
    print(
        "max relative error:",
        (
            (src_output - dst_output).abs()
            / torch.max(src_output.abs(), dst_output.abs())
        ).max(),
    )
